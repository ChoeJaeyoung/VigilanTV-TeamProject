...
while num <= endnum:
    ...
    inputCarDIR = 'output/box_id' + str(idlist[num]) + '_3s.jpg'
    ...
    # 자동차 번호판 자르기##########################################################################################
    detect = carplateDetecting(carIMG, 'darkflow/darkflow/cfg/obj.names', 'darkflow/darkflow/cfg/yolov2-carplate.cfg','darkflow/darkflow/cfg/yolov2-carplate_2200.weights')
    ...
    if result is not None:
        ...
        if bool(re.search('\d{2}\D\s\d{4}', resultOCR)):
            resultOCR = re.search('\d{2}\D\s\d{4}', resultOCR).group()
