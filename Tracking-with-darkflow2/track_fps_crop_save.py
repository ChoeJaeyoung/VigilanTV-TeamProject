...
options = {"model": "darkflow/cfg/yolo.cfg", "load": "darkflow/bin/yolo.weights", "threshold": 0.25}  # threshold가 신뢰율임
...
cap = cv2.VideoCapture('darkflow/test02.MOV')
...
while True:
    ...
    if ret == True:
        ...
        for i in range(num):
            ...
            # 10 프레임 넘어가는 애들은 첫 사진 촬영 (이미지 저장)
            if IDcount[strID] == 10:
                XYbox[strID + "f10"] = [1, trackers_id[i][0], trackers_id[i][1], trackers_id[i][2], trackers_id[i][3]]
                print("XYbox 10프레임 때 캡쳐 된 좌표 추가 : ")
                print(XYbox)
                xy = XYbox[strID + "f10"]
                topx = xy[1]
                topy = xy[2]
                bottomx = xy[3]
                bottomy = xy[4]
                cv2.imwrite("output/img_id" + strID + "_f10" + ".jpg", tracking)
                cropped = frame[int(topy):int(bottomy) + 1, int(topx):int(bottomx) + 1]
                cv2.imwrite("output/box_id" + strID + "_f10" + ".jpg", cropped)

            if IDcount[strID] == 20:
                XYbox[strID + "f20"] = [2, trackers_id[i][0], trackers_id[i][1], trackers_id[i][2], trackers_id[i][3]]
                print("XYbox 20프레임 때 캡쳐 된 좌표 추가 : ")
                print(XYbox)
                xy = XYbox[strID + "f20"]
                topx = xy[1]
                topy = xy[2]
                bottomx = xy[3]
                bottomy = xy[4]
                cv2.imwrite("output/img_id" + strID + "_f20" + ".jpg", tracking)
                cropped = frame[int(topy):int(bottomy) + 1, int(topx):int(bottomx) + 1]
                cv2.imwrite("output/box_id" + strID + "_f20" + ".jpg", cropped)
